...
Both HTTP and Arrow decoding preserve microseconds and support server-computed
values beyond year 9999, including Databend's internal upper bound in year 11000.
Arrow decoding no longer clamps timestamps to Jiff's smaller range. Supporting
these results does not expand the server's accepted calendar-text input range.
...
change in a gap. Arrow timestamps identify the instant unambiguously.
